...
export class FoyerUpdateFormComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private actroute: ActivatedRoute, private foyerService: FoyerService, private route: Router,private snackBar:SnackBarService) { }
  ...
  onSubmit() {
    if (this.foyerForm.valid) {
      this.foyerForm.value.idFoyer = this.idFoyer;
      console.log(this.foyerForm.value);
      this.foyerService.modifierFoyer(this.foyerForm.value).subscribe({
        next: () => {
          this.snackBar.successMessage("Foyer modifié avec succés", "close");
