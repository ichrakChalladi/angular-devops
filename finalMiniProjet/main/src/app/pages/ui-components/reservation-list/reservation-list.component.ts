...
export class ReservationListComponent {
  ...
  ngOnInit(): void {
    this.reservationService.getReservations().subscribe((reservations) => {
      ...
      // Create an array of observables for each getBlocByChambre request
      const blocObservables = this.reservations.map(reservation =>
        this.blocService.getBlocByChambre(reservation.chambre).pipe(
          // Modify data and assign it to the chambre property
          map((data: any) => {
            delete data.chambres;
            reservation.chambre.bloc = data;
          })
        )
      );

      // Use forkJoin to wait for all observables to complete
      forkJoin(blocObservables).subscribe(() => {
        // This code will run after all getBlocByChambre requests have completed
        this.oldResults = this.reservations;
  ...
  performSearch() {
    ...
    if (this.chambreSearchTerms.value !== '') {
      ...
      filteredReservations = filteredReservations.filter(reservation =>
        ...
        reservation.chambre.bloc.nomBloc.toLowerCase().includes(chambreSearchTerm)
      );
    }
    ...
    if (this.typeChambre !== '') {
      filteredReservations = filteredReservations.filter(reservation =>
        reservation.chambre.typec.toLowerCase().includes(this.typeChambre)
      );
    }
